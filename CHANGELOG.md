...
- collapsed `vision_local_core/_legacy.py` into a thin compatibility shell that re-exports the active modules instead of carrying the full runtime implementation
...
- added `vision_local_core/contracts.py` so the public analysis and capability payloads have explicit `TypedDict` contracts
...
- added checked-in PNG fixtures plus mixed-language Tesseract integration coverage, including Ubuntu CI setup for `chi_sim` and CJK fonts
...
- expanded golden regression coverage for low-resolution browser screens, mixed-language settings and line-chart screenshots, and OCR retry recovery on noisy chat captures
- broadened browser, chat, chart, and settings keyword handling for mixed Chinese-English screenshots
- preserved bilingual UI labels in structured layout fields instead of collapsing them into generic English canonical text
